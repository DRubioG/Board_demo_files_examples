-- ==============================================================
-- RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and OpenCL
-- Version: 2019.2
-- Copyright (C) 1986-2019 Xilinx, Inc. All Rights Reserved.
-- 
-- ===========================================================

library IEEE;
use IEEE.std_logic_1164.all;
use IEEE.numeric_std.all;

entity k732_hls_fir_filter_ip is
port (
    ap_clk : IN STD_LOGIC;
    ap_rst : IN STD_LOGIC;
    input_r : IN STD_LOGIC_VECTOR (31 downto 0);
    fir_time : IN STD_LOGIC_VECTOR (7 downto 0);
    output_r : OUT STD_LOGIC_VECTOR (31 downto 0);
    result_valid : OUT STD_LOGIC_VECTOR (7 downto 0) );
end;


architecture behav of k732_hls_fir_filter_ip is 
    attribute CORE_GENERATION_INFO : STRING;
    attribute CORE_GENERATION_INFO of behav : architecture is
    "k732_hls_fir_filter_ip,hls_ip_2019_2,{HLS_INPUT_TYPE=cxx,HLS_INPUT_FLOAT=0,HLS_INPUT_FIXED=0,HLS_INPUT_PART=xc7z010-clg400-1,HLS_INPUT_CLOCK=10.000000,HLS_INPUT_ARCH=others,HLS_SYN_CLOCK=6.794000,HLS_SYN_LAT=-1,HLS_SYN_TPT=none,HLS_SYN_MEM=1,HLS_SYN_DSP=0,HLS_SYN_FF=746,HLS_SYN_LUT=755,HLS_VERSION=2019_2}";
    constant ap_const_logic_1 : STD_LOGIC := '1';
    constant ap_const_logic_0 : STD_LOGIC := '0';
    constant ap_ST_fsm_state1 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000000001";
    constant ap_ST_fsm_state2 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000000010";
    constant ap_ST_fsm_state3 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000000100";
    constant ap_ST_fsm_state4 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000001000";
    constant ap_ST_fsm_state5 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000010000";
    constant ap_ST_fsm_state6 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000100000";
    constant ap_ST_fsm_state7 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000001000000";
    constant ap_ST_fsm_state8 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000010000000";
    constant ap_ST_fsm_state9 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000100000000";
    constant ap_ST_fsm_state10 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000001000000000";
    constant ap_ST_fsm_state11 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000010000000000";
    constant ap_ST_fsm_state12 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000100000000000";
    constant ap_ST_fsm_state13 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000001000000000000";
    constant ap_ST_fsm_state14 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000010000000000000";
    constant ap_ST_fsm_state15 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000100000000000000";
    constant ap_ST_fsm_state16 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000001000000000000000";
    constant ap_ST_fsm_state17 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000010000000000000000";
    constant ap_ST_fsm_state18 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000100000000000000000";
    constant ap_ST_fsm_state19 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000001000000000000000000";
    constant ap_ST_fsm_state20 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000010000000000000000000";
    constant ap_ST_fsm_state21 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000100000000000000000000";
    constant ap_ST_fsm_state22 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000001000000000000000000000";
    constant ap_ST_fsm_state23 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000010000000000000000000000";
    constant ap_ST_fsm_state24 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000100000000000000000000000";
    constant ap_ST_fsm_state25 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000001000000000000000000000000";
    constant ap_ST_fsm_state26 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000010000000000000000000000000";
    constant ap_ST_fsm_state27 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000100000000000000000000000000";
    constant ap_ST_fsm_state28 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000001000000000000000000000000000";
    constant ap_ST_fsm_state29 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000010000000000000000000000000000";
    constant ap_ST_fsm_state30 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000100000000000000000000000000000";
    constant ap_ST_fsm_state31 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000001000000000000000000000000000000";
    constant ap_ST_fsm_state32 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000010000000000000000000000000000000";
    constant ap_ST_fsm_state33 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000100000000000000000000000000000000";
    constant ap_ST_fsm_state34 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000001000000000000000000000000000000000";
    constant ap_ST_fsm_state35 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000010000000000000000000000000000000000";
    constant ap_ST_fsm_state36 : STD_LOGIC_VECTOR (45 downto 0) := "0000000000100000000000000000000000000000000000";
    constant ap_ST_fsm_state37 : STD_LOGIC_VECTOR (45 downto 0) := "0000000001000000000000000000000000000000000000";
    constant ap_ST_fsm_state38 : STD_LOGIC_VECTOR (45 downto 0) := "0000000010000000000000000000000000000000000000";
    constant ap_ST_fsm_state39 : STD_LOGIC_VECTOR (45 downto 0) := "0000000100000000000000000000000000000000000000";
    constant ap_ST_fsm_state40 : STD_LOGIC_VECTOR (45 downto 0) := "0000001000000000000000000000000000000000000000";
    constant ap_ST_fsm_state41 : STD_LOGIC_VECTOR (45 downto 0) := "0000010000000000000000000000000000000000000000";
    constant ap_ST_fsm_state42 : STD_LOGIC_VECTOR (45 downto 0) := "0000100000000000000000000000000000000000000000";
    constant ap_ST_fsm_state43 : STD_LOGIC_VECTOR (45 downto 0) := "0001000000000000000000000000000000000000000000";
    constant ap_ST_fsm_state44 : STD_LOGIC_VECTOR (45 downto 0) := "0010000000000000000000000000000000000000000000";
    constant ap_ST_fsm_state45 : STD_LOGIC_VECTOR (45 downto 0) := "0100000000000000000000000000000000000000000000";
    constant ap_ST_fsm_state46 : STD_LOGIC_VECTOR (45 downto 0) := "1000000000000000000000000000000000000000000000";
    constant ap_const_lv32_0 : STD_LOGIC_VECTOR (31 downto 0) := "00000000000000000000000000000000";
    constant ap_const_lv1_0 : STD_LOGIC_VECTOR (0 downto 0) := "0";
    constant ap_const_lv32_1 : STD_LOGIC_VECTOR (31 downto 0) := "00000000000000000000000000000001";
    constant ap_const_lv1_1 : STD_LOGIC_VECTOR (0 downto 0) := "1";
    constant ap_const_lv32_2 : STD_LOGIC_VECTOR (31 downto 0) := "00000000000000000000000000000010";
    constant ap_const_lv40_0 : STD_LOGIC_VECTOR (39 downto 0) := "0000000000000000000000000000000000000000";
    constant ap_const_lv8_0 : STD_LOGIC_VECTOR (7 downto 0) := "00000000";
    constant ap_const_lv32_2D : STD_LOGIC_VECTOR (31 downto 0) := "00000000000000000000000000101101";
    constant ap_const_lv32_3 : STD_LOGIC_VECTOR (31 downto 0) := "00000000000000000000000000000011";
    constant ap_const_lv8_C8 : STD_LOGIC_VECTOR (7 downto 0) := "11001000";
    constant ap_const_lv8_1 : STD_LOGIC_VECTOR (7 downto 0) := "00000001";
    constant ap_const_lv9_1FF : STD_LOGIC_VECTOR (8 downto 0) := "111111111";
    constant ap_const_boolean_1 : BOOLEAN := true;

    signal filter_ptr : STD_LOGIC_VECTOR (31 downto 0) := "00000000000000000000000000000000";
    signal buffer_in_address0 : STD_LOGIC_VECTOR (7 downto 0);
    signal buffer_in_ce0 : STD_LOGIC;
    signal buffer_in_we0 : STD_LOGIC;
    signal buffer_in_q0 : STD_LOGIC_VECTOR (31 downto 0);
    signal result_valid_flag : STD_LOGIC_VECTOR (0 downto 0) := "0";
    signal icmp_ln19_fu_152_p2 : STD_LOGIC_VECTOR (0 downto 0);
    signal icmp_ln19_reg_280 : STD_LOGIC_VECTOR (0 downto 0);
    signal ap_CS_fsm : STD_LOGIC_VECTOR (45 downto 0) := "0000000000000000000000000000000000000000000001";
    attribute fsm_encoding : string;
    attribute fsm_encoding of ap_CS_fsm : signal is "none";
    signal ap_CS_fsm_state1 : STD_LOGIC;
    attribute fsm_encoding of ap_CS_fsm_state1 : signal is "none";
    signal select_ln26_fu_164_p3 : STD_LOGIC_VECTOR (7 downto 0);
    signal select_ln26_reg_284 : STD_LOGIC_VECTOR (7 downto 0);
    signal zext_ln26_fu_172_p1 : STD_LOGIC_VECTOR (8 downto 0);
    signal zext_ln26_reg_289 : STD_LOGIC_VECTOR (8 downto 0);
    signal filter_ptr_load_reg_294 : STD_LOGIC_VECTOR (31 downto 0);
    signal select_ln28_fu_191_p3 : STD_LOGIC_VECTOR (7 downto 0);
    signal select_ln28_reg_299 : STD_LOGIC_VECTOR (7 downto 0);
    signal i_fu_204_p2 : STD_LOGIC_VECTOR (7 downto 0);
    signal i_reg_307 : STD_LOGIC_VECTOR (7 downto 0);
    signal ap_CS_fsm_state2 : STD_LOGIC;
    attribute fsm_encoding of ap_CS_fsm_state2 : signal is "none";
    signal icmp_ln28_fu_199_p2 : STD_LOGIC_VECTOR (0 downto 0);
    signal add_ln29_fu_215_p2 : STD_LOGIC_VECTOR (31 downto 0);
    signal tmp_add_value_fu_254_p2 : STD_LOGIC_VECTOR (39 downto 0);
    signal ap_CS_fsm_state3 : STD_LOGIC;
    attribute fsm_encoding of ap_CS_fsm_state3 : signal is "none";
    signal tmp_add_value_0_reg_106 : STD_LOGIC_VECTOR (39 downto 0);
    signal i_0_reg_118 : STD_LOGIC_VECTOR (7 downto 0);
    signal filter_ptr_new_0_reg_129 : STD_LOGIC_VECTOR (31 downto 0);
    signal icmp_ln30_fu_229_p2 : STD_LOGIC_VECTOR (0 downto 0);
    signal trunc_ln36_fu_275_p1 : STD_LOGIC_VECTOR (31 downto 0);
    signal ap_phi_mux_storemerge_phi_fu_144_p4 : STD_LOGIC_VECTOR (31 downto 0);
    signal storemerge_reg_140 : STD_LOGIC_VECTOR (31 downto 0);
    signal ap_CS_fsm_state46 : STD_LOGIC;
    attribute fsm_encoding of ap_CS_fsm_state46 : signal is "none";
    signal sext_ln27_fu_180_p1 : STD_LOGIC_VECTOR (63 downto 0);
    signal zext_ln28_fu_210_p1 : STD_LOGIC_VECTOR (63 downto 0);
    signal ap_CS_fsm_state4 : STD_LOGIC;
    attribute fsm_encoding of ap_CS_fsm_state4 : signal is "none";
    signal icmp_ln26_fu_158_p2 : STD_LOGIC_VECTOR (0 downto 0);
    signal sext_ln27_fu_180_p0 : STD_LOGIC_VECTOR (31 downto 0);
    signal empty_fu_185_p2 : STD_LOGIC_VECTOR (0 downto 0);
    signal add_ln30_fu_220_p2 : STD_LOGIC_VECTOR (8 downto 0);
    signal sext_ln30_fu_225_p1 : STD_LOGIC_VECTOR (31 downto 0);
    signal grp_fu_244_p1 : STD_LOGIC_VECTOR (7 downto 0);
    signal zext_ln28_1_fu_250_p1 : STD_LOGIC_VECTOR (39 downto 0);
    signal grp_fu_244_p2 : STD_LOGIC_VECTOR (31 downto 0);
    signal grp_fu_244_ap_start : STD_LOGIC;
    signal grp_fu_244_ap_done : STD_LOGIC;
    signal ap_NS_fsm : STD_LOGIC_VECTOR (45 downto 0);
    signal grp_fu_244_p10 : STD_LOGIC_VECTOR (39 downto 0);

    component k732_hls_fir_filtcud IS
    generic (
        ID : INTEGER;
        NUM_STAGE : INTEGER;
        din0_WIDTH : INTEGER;
        din1_WIDTH : INTEGER;
        dout_WIDTH : INTEGER );
    port (
        clk : IN STD_LOGIC;
        reset : IN STD_LOGIC;
        start : IN STD_LOGIC;
        done : OUT STD_LOGIC;
        din0 : IN STD_LOGIC_VECTOR (39 downto 0);
        din1 : IN STD_LOGIC_VECTOR (7 downto 0);
        ce : IN STD_LOGIC;
        dout : OUT STD_LOGIC_VECTOR (31 downto 0) );
    end component;


    component k732_hls_fir_filtbkb IS
    generic (
        DataWidth : INTEGER;
        AddressRange : INTEGER;
        AddressWidth : INTEGER );
    port (
        clk : IN STD_LOGIC;
        reset : IN STD_LOGIC;
        address0 : IN STD_LOGIC_VECTOR (7 downto 0);
        ce0 : IN STD_LOGIC;
        we0 : IN STD_LOGIC;
        d0 : IN STD_LOGIC_VECTOR (31 downto 0);
        q0 : OUT STD_LOGIC_VECTOR (31 downto 0) );
    end component;



begin
    buffer_in_U : component k732_hls_fir_filtbkb
    generic map (
        DataWidth => 32,
        AddressRange => 256,
        AddressWidth => 8)
    port map (
        clk => ap_clk,
        reset => ap_rst,
        address0 => buffer_in_address0,
        ce0 => buffer_in_ce0,
        we0 => buffer_in_we0,
        d0 => input_r,
        q0 => buffer_in_q0);

    k732_hls_fir_filtcud_U1 : component k732_hls_fir_filtcud
    generic map (
        ID => 1,
        NUM_STAGE => 44,
        din0_WIDTH => 40,
        din1_WIDTH => 8,
        dout_WIDTH => 32)
    port map (
        clk => ap_clk,
        reset => ap_rst,
        start => grp_fu_244_ap_start,
        done => grp_fu_244_ap_done,
        din0 => tmp_add_value_0_reg_106,
        din1 => grp_fu_244_p1,
        ce => ap_const_logic_1,
        dout => grp_fu_244_p2);





    ap_CS_fsm_assign_proc : process(ap_clk)
    begin
        if (ap_clk'event and ap_clk =  '1') then
            if (ap_rst = '1') then
                ap_CS_fsm <= ap_ST_fsm_state1;
            else
                ap_CS_fsm <= ap_NS_fsm;
            end if;
        end if;
    end process;


    filter_ptr_new_0_reg_129_assign_proc : process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if (((icmp_ln28_fu_199_p2 = ap_const_lv1_1) and (ap_const_logic_1 = ap_CS_fsm_state2))) then
                if ((icmp_ln30_fu_229_p2 = ap_const_lv1_0)) then 
                    filter_ptr_new_0_reg_129 <= add_ln29_fu_215_p2;
                elsif ((icmp_ln30_fu_229_p2 = ap_const_lv1_1)) then 
                    filter_ptr_new_0_reg_129 <= ap_const_lv32_0;
                end if;
            end if; 
        end if;
    end process;

    i_0_reg_118_assign_proc : process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if ((ap_const_logic_1 = ap_CS_fsm_state3)) then 
                i_0_reg_118 <= i_reg_307;
            elsif (((icmp_ln19_fu_152_p2 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state1))) then 
                i_0_reg_118 <= ap_const_lv8_0;
            end if; 
        end if;
    end process;

    storemerge_reg_140_assign_proc : process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if (((icmp_ln19_fu_152_p2 = ap_const_lv1_1) and (ap_const_logic_1 = ap_CS_fsm_state1))) then 
                storemerge_reg_140 <= ap_const_lv32_0;
            elsif (((icmp_ln19_reg_280 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state46))) then 
                storemerge_reg_140 <= trunc_ln36_fu_275_p1;
            end if; 
        end if;
    end process;

    tmp_add_value_0_reg_106_assign_proc : process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if ((ap_const_logic_1 = ap_CS_fsm_state3)) then 
                tmp_add_value_0_reg_106 <= tmp_add_value_fu_254_p2;
            elsif (((icmp_ln19_fu_152_p2 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state1))) then 
                tmp_add_value_0_reg_106 <= ap_const_lv40_0;
            end if; 
        end if;
    end process;
    process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if ((ap_const_logic_1 = ap_CS_fsm_state4)) then
                filter_ptr <= filter_ptr_new_0_reg_129;
            end if;
        end if;
    end process;
    process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if (((icmp_ln19_fu_152_p2 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state1))) then
                filter_ptr_load_reg_294 <= filter_ptr;
                select_ln26_reg_284 <= select_ln26_fu_164_p3;
                select_ln28_reg_299 <= select_ln28_fu_191_p3;
                    zext_ln26_reg_289(7 downto 0) <= zext_ln26_fu_172_p1(7 downto 0);
            end if;
        end if;
    end process;
    process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if ((ap_const_logic_1 = ap_CS_fsm_state2)) then
                i_reg_307 <= i_fu_204_p2;
            end if;
        end if;
    end process;
    process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if ((ap_const_logic_1 = ap_CS_fsm_state1)) then
                icmp_ln19_reg_280 <= icmp_ln19_fu_152_p2;
            end if;
        end if;
    end process;
    process (ap_clk)
    begin
        if (ap_clk'event and ap_clk = '1') then
            if (((icmp_ln28_fu_199_p2 = ap_const_lv1_1) and (ap_const_logic_1 = ap_CS_fsm_state2) and (icmp_ln30_fu_229_p2 = ap_const_lv1_1))) then
                result_valid_flag <= ap_const_lv1_1;
            end if;
        end if;
    end process;
    zext_ln26_reg_289(8) <= '0';

    ap_NS_fsm_assign_proc : process (icmp_ln19_fu_152_p2, ap_CS_fsm, ap_CS_fsm_state1, ap_CS_fsm_state2, icmp_ln28_fu_199_p2)
    begin
        case ap_CS_fsm is
            when ap_ST_fsm_state1 => 
                if (((icmp_ln19_fu_152_p2 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state1))) then
                    ap_NS_fsm <= ap_ST_fsm_state2;
                else
                    ap_NS_fsm <= ap_ST_fsm_state46;
                end if;
            when ap_ST_fsm_state2 => 
                if (((icmp_ln28_fu_199_p2 = ap_const_lv1_1) and (ap_const_logic_1 = ap_CS_fsm_state2))) then
                    ap_NS_fsm <= ap_ST_fsm_state4;
                else
                    ap_NS_fsm <= ap_ST_fsm_state3;
                end if;
            when ap_ST_fsm_state3 => 
                ap_NS_fsm <= ap_ST_fsm_state2;
            when ap_ST_fsm_state4 => 
                ap_NS_fsm <= ap_ST_fsm_state5;
            when ap_ST_fsm_state5 => 
                ap_NS_fsm <= ap_ST_fsm_state6;
            when ap_ST_fsm_state6 => 
                ap_NS_fsm <= ap_ST_fsm_state7;
            when ap_ST_fsm_state7 => 
                ap_NS_fsm <= ap_ST_fsm_state8;
            when ap_ST_fsm_state8 => 
                ap_NS_fsm <= ap_ST_fsm_state9;
            when ap_ST_fsm_state9 => 
                ap_NS_fsm <= ap_ST_fsm_state10;
            when ap_ST_fsm_state10 => 
                ap_NS_fsm <= ap_ST_fsm_state11;
            when ap_ST_fsm_state11 => 
                ap_NS_fsm <= ap_ST_fsm_state12;
            when ap_ST_fsm_state12 => 
                ap_NS_fsm <= ap_ST_fsm_state13;
            when ap_ST_fsm_state13 => 
                ap_NS_fsm <= ap_ST_fsm_state14;
            when ap_ST_fsm_state14 => 
                ap_NS_fsm <= ap_ST_fsm_state15;
            when ap_ST_fsm_state15 => 
                ap_NS_fsm <= ap_ST_fsm_state16;
            when ap_ST_fsm_state16 => 
                ap_NS_fsm <= ap_ST_fsm_state17;
            when ap_ST_fsm_state17 => 
                ap_NS_fsm <= ap_ST_fsm_state18;
            when ap_ST_fsm_state18 => 
                ap_NS_fsm <= ap_ST_fsm_state19;
            when ap_ST_fsm_state19 => 
                ap_NS_fsm <= ap_ST_fsm_state20;
            when ap_ST_fsm_state20 => 
                ap_NS_fsm <= ap_ST_fsm_state21;
            when ap_ST_fsm_state21 => 
                ap_NS_fsm <= ap_ST_fsm_state22;
            when ap_ST_fsm_state22 => 
                ap_NS_fsm <= ap_ST_fsm_state23;
            when ap_ST_fsm_state23 => 
                ap_NS_fsm <= ap_ST_fsm_state24;
            when ap_ST_fsm_state24 => 
                ap_NS_fsm <= ap_ST_fsm_state25;
            when ap_ST_fsm_state25 => 
                ap_NS_fsm <= ap_ST_fsm_state26;
            when ap_ST_fsm_state26 => 
                ap_NS_fsm <= ap_ST_fsm_state27;
            when ap_ST_fsm_state27 => 
                ap_NS_fsm <= ap_ST_fsm_state28;
            when ap_ST_fsm_state28 => 
                ap_NS_fsm <= ap_ST_fsm_state29;
            when ap_ST_fsm_state29 => 
                ap_NS_fsm <= ap_ST_fsm_state30;
            when ap_ST_fsm_state30 => 
                ap_NS_fsm <= ap_ST_fsm_state31;
            when ap_ST_fsm_state31 => 
                ap_NS_fsm <= ap_ST_fsm_state32;
            when ap_ST_fsm_state32 => 
                ap_NS_fsm <= ap_ST_fsm_state33;
            when ap_ST_fsm_state33 => 
                ap_NS_fsm <= ap_ST_fsm_state34;
            when ap_ST_fsm_state34 => 
                ap_NS_fsm <= ap_ST_fsm_state35;
            when ap_ST_fsm_state35 => 
                ap_NS_fsm <= ap_ST_fsm_state36;
            when ap_ST_fsm_state36 => 
                ap_NS_fsm <= ap_ST_fsm_state37;
            when ap_ST_fsm_state37 => 
                ap_NS_fsm <= ap_ST_fsm_state38;
            when ap_ST_fsm_state38 => 
                ap_NS_fsm <= ap_ST_fsm_state39;
            when ap_ST_fsm_state39 => 
                ap_NS_fsm <= ap_ST_fsm_state40;
            when ap_ST_fsm_state40 => 
                ap_NS_fsm <= ap_ST_fsm_state41;
            when ap_ST_fsm_state41 => 
                ap_NS_fsm <= ap_ST_fsm_state42;
            when ap_ST_fsm_state42 => 
                ap_NS_fsm <= ap_ST_fsm_state43;
            when ap_ST_fsm_state43 => 
                ap_NS_fsm <= ap_ST_fsm_state44;
            when ap_ST_fsm_state44 => 
                ap_NS_fsm <= ap_ST_fsm_state45;
            when ap_ST_fsm_state45 => 
                ap_NS_fsm <= ap_ST_fsm_state46;
            when ap_ST_fsm_state46 => 
                ap_NS_fsm <= ap_ST_fsm_state1;
            when others =>  
                ap_NS_fsm <= "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
        end case;
    end process;
    add_ln29_fu_215_p2 <= std_logic_vector(signed(filter_ptr_load_reg_294) + signed(ap_const_lv32_1));
    add_ln30_fu_220_p2 <= std_logic_vector(unsigned(zext_ln26_reg_289) + unsigned(ap_const_lv9_1FF));
    ap_CS_fsm_state1 <= ap_CS_fsm(0);
    ap_CS_fsm_state2 <= ap_CS_fsm(1);
    ap_CS_fsm_state3 <= ap_CS_fsm(2);
    ap_CS_fsm_state4 <= ap_CS_fsm(3);
    ap_CS_fsm_state46 <= ap_CS_fsm(45);

    ap_phi_mux_storemerge_phi_fu_144_p4_assign_proc : process(icmp_ln19_reg_280, trunc_ln36_fu_275_p1, storemerge_reg_140, ap_CS_fsm_state46)
    begin
        if (((icmp_ln19_reg_280 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state46))) then 
            ap_phi_mux_storemerge_phi_fu_144_p4 <= trunc_ln36_fu_275_p1;
        else 
            ap_phi_mux_storemerge_phi_fu_144_p4 <= storemerge_reg_140;
        end if; 
    end process;


    buffer_in_address0_assign_proc : process(ap_CS_fsm_state1, ap_CS_fsm_state2, sext_ln27_fu_180_p1, zext_ln28_fu_210_p1)
    begin
        if ((ap_const_logic_1 = ap_CS_fsm_state2)) then 
            buffer_in_address0 <= zext_ln28_fu_210_p1(8 - 1 downto 0);
        elsif ((ap_const_logic_1 = ap_CS_fsm_state1)) then 
            buffer_in_address0 <= sext_ln27_fu_180_p1(8 - 1 downto 0);
        else 
            buffer_in_address0 <= "XXXXXXXX";
        end if; 
    end process;


    buffer_in_ce0_assign_proc : process(ap_CS_fsm_state1, ap_CS_fsm_state2)
    begin
        if (((ap_const_logic_1 = ap_CS_fsm_state2) or (ap_const_logic_1 = ap_CS_fsm_state1))) then 
            buffer_in_ce0 <= ap_const_logic_1;
        else 
            buffer_in_ce0 <= ap_const_logic_0;
        end if; 
    end process;


    buffer_in_we0_assign_proc : process(icmp_ln19_fu_152_p2, ap_CS_fsm_state1)
    begin
        if (((icmp_ln19_fu_152_p2 = ap_const_lv1_0) and (ap_const_logic_1 = ap_CS_fsm_state1))) then 
            buffer_in_we0 <= ap_const_logic_1;
        else 
            buffer_in_we0 <= ap_const_logic_0;
        end if; 
    end process;

    empty_fu_185_p2 <= "1" when (unsigned(fir_time) < unsigned(ap_const_lv8_C8)) else "0";

    grp_fu_244_ap_start_assign_proc : process(ap_CS_fsm_state2, icmp_ln28_fu_199_p2)
    begin
        if (((icmp_ln28_fu_199_p2 = ap_const_lv1_1) and (ap_const_logic_1 = ap_CS_fsm_state2))) then 
            grp_fu_244_ap_start <= ap_const_logic_1;
        else 
            grp_fu_244_ap_start <= ap_const_logic_0;
        end if; 
    end process;

    grp_fu_244_p1 <= grp_fu_244_p10(8 - 1 downto 0);
    grp_fu_244_p10 <= std_logic_vector(IEEE.numeric_std.resize(unsigned(select_ln26_reg_284),40));
    i_fu_204_p2 <= std_logic_vector(unsigned(i_0_reg_118) + unsigned(ap_const_lv8_1));
    icmp_ln19_fu_152_p2 <= "1" when (fir_time = ap_const_lv8_0) else "0";
    icmp_ln26_fu_158_p2 <= "1" when (unsigned(fir_time) > unsigned(ap_const_lv8_C8)) else "0";
    icmp_ln28_fu_199_p2 <= "1" when (i_0_reg_118 = select_ln28_reg_299) else "0";
    icmp_ln30_fu_229_p2 <= "1" when (signed(add_ln29_fu_215_p2) > signed(sext_ln30_fu_225_p1)) else "0";
    output_r <= ap_phi_mux_storemerge_phi_fu_144_p4;
    result_valid <= std_logic_vector(IEEE.numeric_std.resize(unsigned(result_valid_flag),8));
    select_ln26_fu_164_p3 <= 
        ap_const_lv8_C8 when (icmp_ln26_fu_158_p2(0) = '1') else 
        fir_time;
    select_ln28_fu_191_p3 <= 
        fir_time when (empty_fu_185_p2(0) = '1') else 
        ap_const_lv8_C8;
    sext_ln27_fu_180_p0 <= filter_ptr;
        sext_ln27_fu_180_p1 <= std_logic_vector(IEEE.numeric_std.resize(signed(sext_ln27_fu_180_p0),64));

        sext_ln30_fu_225_p1 <= std_logic_vector(IEEE.numeric_std.resize(signed(add_ln30_fu_220_p2),32));

    tmp_add_value_fu_254_p2 <= std_logic_vector(unsigned(zext_ln28_1_fu_250_p1) + unsigned(tmp_add_value_0_reg_106));
    trunc_ln36_fu_275_p1 <= grp_fu_244_p2(32 - 1 downto 0);
    zext_ln26_fu_172_p1 <= std_logic_vector(IEEE.numeric_std.resize(unsigned(select_ln26_fu_164_p3),9));
    zext_ln28_1_fu_250_p1 <= std_logic_vector(IEEE.numeric_std.resize(unsigned(buffer_in_q0),40));
    zext_ln28_fu_210_p1 <= std_logic_vector(IEEE.numeric_std.resize(unsigned(i_0_reg_118),64));
end behav;
