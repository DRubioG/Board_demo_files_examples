...
parameter IIC_REF_CLK_CNT=3000;
 wire iic_ref_clk;
 reg clock_iic_ref_reg;
 reg [8:0]reg_index; 
 reg [15:0]clock_iic_ref_cnt;
 reg [1:0]config_step;
 ...
 assign iic_ref_clk=clock_iic_ref_reg;
 i2c_driver i2c_driver_inst1(
           .clock_i2c(iic_ref_clk),
...
//产生ii2参考时钟 CLK_REF 
always@(posedge clk or negedge camera_rstn)   
begin
   if(!camera_rstn) begin
        clock_iic_ref_reg<=0;
        clock_iic_ref_cnt<=0;
   end
   else if(clock_iic_ref_cnt<IIC_REF_CLK_CNT)
      clock_iic_ref_cnt<=clock_iic_ref_cnt+1'b1;
   else begin
         clock_iic_ref_reg<=!clock_iic_ref_reg;
         clock_iic_ref_cnt<=0;
   end
end


always@(posedge iic_ref_clk or negedge camera_rstn)    
begin
   if(!camera_rstn) begin
       config_step<=0;
       start<=0;
      ...
      if(reg_conf_done_reg==1'b0) begin          // NOT DONE
			  if(reg_index<302) begin
					 case(config_step)
					 0:begin
						i2c_data<={8'h78,reg_data};       //IIC Device addr 0x78   
						start<=1;
						config_step<=1;
					 end
					 1:begin
						if(done) begin                       //IIC send done            					
							 start<=0;
							 config_step<=2;
						end
					 end
...
always@(reg_index)   
 begin
    case(reg_index)
	 ...
	 36:reg_data<=24'h362201; // 50/60Hz detection 
