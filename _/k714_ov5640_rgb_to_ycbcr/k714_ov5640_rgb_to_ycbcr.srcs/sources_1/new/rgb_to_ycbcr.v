...
(
   ...
   output wire cmos_conf_done2,
    ...
    (*mark_debug="true"*)wire [15:0]cmos_vedio_data2;
    wire cmos_vedio_wen1;
    wire cmos_vedio_wen2;
    wire [15:0]ram_read_port_data;
...
//for calculate
//实际上YCBCR不能直接作为信号输入到HDMI
//我们最后只取Y分量，在另一个HDMI显示黑白，Y需要给HDMI输入RGB三个分量
//这里只展示CB CR计算
...
rgb2dvi_0 rgb2dvi_0_inst1
  (
   ...
   (
    ...
    .aRst(1'b0), 
    .vid_pData({ycbcr_y, ycbcr_y, ycbcr_y}),
    .vid_pVDE(ready),
    .vid_pHSync(Hsync),
    .vid_pVSync(Vsync),
    .PixelClk(clk_74p25MHz),
