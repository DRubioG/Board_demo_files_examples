...
begin
	if(!rstn)
		...
		begin
		if(~cmos_vsync & cmos_href)			//行场有效，{HIGH BYTE, LOW BYTE} 
			begin
			byte_state <= byte_state + 1'b1;	//（RGB565 = {HIGH BYTE, LOW BYTE} 
			case(byte_state)
...
begin
	...
	else if(init_done_flag)					//CMOS I2C初始化结束
		begin
		if(cmos_vsync_over == 1'b1)		//帧结束
			begin
			if(frame_cont < 12)
				begin
...
begin
	if(!rstn)
		cmos_oclk <= 0;
	else if(frame_valid == 1'b1 && byte_state)//(X_Cont >=1 && X_Cont <= H_DISP))
		cmos_oclk <= ~cmos_oclk;
	else
		cmos_oclk <= 0;
end
